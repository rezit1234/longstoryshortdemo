...
export function AdminPdfCodePositionEditor({
  pdfUrl,
  initialPosition,
  onClose,
  onSave,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Nastavení pozice kódu na poukazu"
      onMouseDown={(event) => {
    ...
    >
      <div className="admin-code-editor-panel">
        <div className="admin-code-editor-head">
          <div>
            <p className="admin-code-editor-kicker">Pozice kódu</p>
            <p className="admin-code-editor-lead">
              Přetáhněte rámeček na poukaz a upravte velikost. Ukázka kódu:{" "}
              <strong>{MOCK_VOUCHER_CODE}</strong>
            </p>
          </div>
          <AdminDismissButton label="Zavřít editor pozice" onClick={onClose} />
        </div>
              ...
              >
                ...
                {stageSize.width > 0 ? (
                  <div className="admin-code-editor-overlay">
                    <div
                      className="admin-code-editor-box"
                      style={{
                        left: `${rect.x}%`,
                        top: `${rect.y}%`,
                        width: `${rect.width}%`,
                    ...
                    >
                      <span
                        className="admin-code-editor-box-label"
                        style={{
                          fontSize: `${fitCodeFontSizePx(
                            MOCK_VOUCHER_CODE,
                            (stageSize.width * rect.width) / 100,
                            (stageSize.height * rect.height) / 100,
                          )}px`,
                        }}
                      >
                        {MOCK_VOUCHER_CODE}
